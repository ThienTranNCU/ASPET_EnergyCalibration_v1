//BinaryRead9-LAB240506_EnergyCalCorrection
//Pick up the highest peak in the spectrum and compared with that from the calibration file in keV
//output file:   gmslID / sticID / channel / P0_updated/ P0err / P1 /P1err / P2 / P2err /Chi2 (same with the original calibration file)
//Out put file in Energy_calibration_updated.txt file of the same directory in CalUpdate foulder
...
int main(int argc, char *argv[])
{
    ...
    char argName[1000], fileName[1000], file[10][1000], fileNameRoot[1000],Calname[1000];//file name string
    const int N=20000;
    double xmin=0, xmax=32800,realE=306.82,peakLT=100,peakHT=100000;
    ...
    if(argc>=4){xmin=strtod(argv[3],NULL); xmax=strtod(argv[4],NULL);} //ENERGY LIMITS for ToT ADC
    if(argc>=5){sigP=strtod(argv[5],NULL);} //Peak finding parameter
    if(argc>=6){sigB=strtod(argv[6],NULL);} //Back ground fitting parameter
    if(argc>=7){realE=strtod(argv[7],NULL);}
	if(argc>=9){peakLT=strtod(argv[8],NULL); peakHT=strtod(argv[9],NULL);}  ////Lower and Higher threshold for peak count
...
//    if(argc>=12){pos=strtod(argv[11],NULL);}
 ...
 //   for(Int_t i=0;i<1024;i++){
    ...
    sprintf(fileNameRoot,"mkdir CalUpdate");
    system(fileNameRoot);
    sprintf(fileNameRoot,"mkdir CalUpdate/Spectrum");
    system(fileNameRoot);
    // Defining TFile and tree
/*
    sprintf(fileNameRoot,"CalUpdate/All_outputTree.root");
...
//        printf("Failed opening the LOR write file %s\n",fileNameLOR);
 ...
 //   }
  //  printf("Opening file.. \t %s\n",fileNameLOR);
    ...
    TH2F *hS0 =  new TH2F("hS0","Board 0 - Energy Shift (keV)",32,0,32,16,0,16);
    TH2F *hS1 =  new TH2F("hS1","Board 1 - Energy Shift (keV)",32,0,32,16,0,16);
    ...
    TH1F *hES =  new TH1F("hES","Energy Shift",400,-200.0,200.0);
 ...
 char fileNamePeak[1000],fileNamePeak2[1000];
    sprintf(fileNamePeak,"CalUpdate/HighestPeak.txt");

ofstream out;

out.open(Form(fileNamePeak));

sprintf(fileNamePeak2,"CalUpdate/EnergyShift.txt");

ofstream out1;

out1.open(Form(fileNamePeak2));

sprintf(fileNamePeak2,"CalUpdate/Energy_calibration_updated.txt");

ofstream out2;

out2.open(Form(fileNamePeak2));
    ////////////////////////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for(int g1=0; g1<4; g1++)
    {
        ...
        //        cout<<"Fitting parameters: "<< g1<<"\t"<<s1<<"\t"<<i<<"\t"<<"  #########   "<<fG->GetParameter(0)<<"\t"<<fG->GetParameter(1)<<"\t"<<fG->GetParameter(2)<<endl;
          ...
          if((g1==0)||(g1==1)){
			...
			if(((a>0)&&(a<=7))&&((abs(fG->GetParameter(2))<sigB*2))&&((peak>=peakLT)&&(peak<=peakHT))&&(Kill1[g1][s1][i]<50)){                                                                                    //// condition for good spectrum and good fit
                out<<g1<<"\t"<<s1<<"\t"<<i<<"\t"<<peak<<"\t"<<fG->GetParameter(1)<<"\t"<<abs(fG->GetParameter(2))<<"\t"<<realE<<endl;
                out1<<g1<<"\t"<<s1<<"\t"<<i<<"\t"<<(CalE((fG->GetParameter(1)),g1,s1,i,par)-realE)<<endl;
                P0c[g1][s1][i]=par[g1][s1][i][0]/(exp((CalE((fG->GetParameter(1)),g1,s1,i,par)-realE)/(par[g1][s1][i][1])));
                hES->Fill(CalE((fG->GetParameter(1)),g1,s1,i,par)-realE);
				if((g1==0)||(g1==1)){
					hPeakC0->SetBinContent(px1+1,py1+1,peak);
					hS0->SetBinContent(px1+1,py1+1,(CalE((fG->GetParameter(1)),g1,s1,i,par)-realE));
				}else{
					hPeakC1->SetBinContent(px1+1,py1+1,peak);	
					hS1->SetBinContent(px1+1,py1+1,(CalE((fG->GetParameter(1)),g1,s1,i,par)-realE));
				}
			}else{
				out<<g1<<"\t"<<s1<<"\t"<<i<<"\t"<<0.0<<"\t"<<0.0<<"\t"<<0.0<<"\t"<<0.0<<endl;
				out1<<g1<<"\t"<<s1<<"\t"<<i<<"\t"<<10000<<endl;
				if((g1==0)||(g1==1)){
    ...
    for(int g=0; g<4; g++)
    {
        for(int s=0;  s<4; s++)
        {
            ...
            for(int j=0; j<4; j++)
            {				
                sprintf(hname,"CalUpdate/Spectrum/G%dS%dE-ChSet%d.root",g,s,j);
            ...
            //    sprintf(hname,"%s.pdf",hname);
               // cE[g][s][j]->Write(); 
                cE[g][s][j]->SaveAs(hname);
                sprintf(hname,"CalUpdate/Spectrum/G%dS%dE-ChSet%d.png",g,s,j);
               // cE[g][s][j]->Write(); 
                cE[g][s][j]->SaveAs(hname);
            }
        }
  ...
  for(int g=0; g<4; g++)
    {
        for(int s=0;  s<4; s++)
        {
            ...
            {		
				...
				out2<<g<<"\t"<<s<<"\t"<<j<<"\t"<<P0c[g][s][j]<<"\t"<<parE[g][s][j][0]<<"\t"<<par[g][s][j][1]<<"\t"<<parE[g][s][j][1]<<"\t"<<par[g][s][j][2]<<"\t"<<parE[g][s][j][2]<<"\t"<<Kill1[g][s][j]<<endl;
			}
		}
	}  
    ...
    //sprintf(fhistname,"CalUpdate/h2Map.pdf",windowTcc, windowEvents,windowcluster);
    ...
    cSall->SaveAs("CalUpdate/SingleAll_v1.root");
    cSall->SaveAs("CalUpdate/SingleAll_v1.png");
  //  cSall->Write();
    ...
    cpeak->SaveAs("CalUpdate/Peak_2D_v1.root");
    cpeak->SaveAs("CalUpdate/Peak_2D_v1.png");
  //  cpeak->Write();
    ...
    cpeakC->SaveAs("CalUpdate/Peak_Selected_2D_v1.root");
    cpeakC->SaveAs("CalUpdate/Peak_Selected_2D_v1.png");
  //  cpeakC->Write();
    ...
    cRe->SaveAs("CalUpdate/Resolution_2D_v1.root");
    cRe->SaveAs("CalUpdate/Resolution_2D_v1.png");
  //  cRe->Write();
    
    TCanvas *cES2= new TCanvas("cES2","cES2",1000,1000);
    gStyle->SetPalette(55);
    cES2->Divide(1,2);
    cES2->cd(1);
    hS0->Draw("colz");
  //  hS0->SetMaximum(0.1);
  //  hS0->SetMinimum(0.0);
    hS0->GetXaxis()->SetTitle("X index");
    hS0->GetYaxis()->SetTitle("Y index");
     cES2->cd(2);
//    hS1->SetMaximum(.1);
//    hS1->SetMinimum(0.0);
    hS1->Draw("colz");
    hS1->GetXaxis()->SetTitle("X index");
    hS1->GetYaxis()->SetTitle("Y index");
    cES2->SaveAs("CalUpdate/EnergyShift_2D_v1.root");
    cES2->SaveAs("CalUpdate/EnergyShift_2D_v1.png");
  //  cES2->Write();
    
    TCanvas *cR= new TCanvas("cR","cR",1000,1000);
    hRes->Draw();
    cR->SaveAs("CalUpdate/Resolution_Distribution_v1.root");
    cR->SaveAs("CalUpdate/Resolution_Distribution_v1.png");
    cR->Write();
    TCanvas *cES= new TCanvas("cES","cES",1000,1000);
    hES->Draw();
    cES->SaveAs("CalUpdate/EnergyShift_v1.root");
    cES->SaveAs("CalUpdate/EnergyShift_v1.png");
  //  cES->Write();
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    //DELETING HISTOGRAMS
    ////////////////////////////////////////////////////////////////////////////////////////////////////
