...
int main(int argc, char *argv[])
{
    ...
    const int N=20000;
    float xmin=0, xmax=32800,realE=306.82,peakLT=100,peakHT=100000,realE2=201.83;
    ...
    if(argc>=7){realE2=strtod(argv[7],NULL);}
...
//        printf("Failed opening the LOR write file %s\n",fileNameLOR);
 ...
 //   }
  //  printf("Opening file.. \t %s\n",fileNameLOR);
    ...
    TH2F *hPeakC0 =  new TH2F("hPeakC0","Selected 1st peak counts board 0",32,0,32,16,0,16);
    TH2F *hPeakC1 =  new TH2F("hPeakC1","Selected 1st peak counts board 1",32,0,32,16,0,16);
    
    TH2F *h2PeakC0 =  new TH2F("h2PeakC0","Selected 2nd peak counts board 0",32,0,32,16,0,16);
    TH2F *h2PeakC1 =  new TH2F("h2PeakC1","Selected 2nd peak counts board 1",32,0,32,16,0,16);
 ...
 char fileNamePeak[1000];
    sprintf(fileNamePeak,"%s_HighestPeak_v2.txt",argName);
ofstream out;
out.open(Form(fileNamePeak));

sprintf(fileNamePeak,"%s_SecondHighestPeak_v2.txt",argName);
ofstream out2;
out2.open(Form(fileNamePeak));
    ////////////////////////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for(int g1=0; g1<4; g1++)
    {
        for(int s1=0; s1<4; s1++)
        {
            ...
            for(int i=0; i<64; i++)
            {
                ...
                std::qsort(parray, a, 2*sizeof(int), compareY); //SORT: X: peak position
    ...
    //               cout<< parray[j][0]<<"\t"<< parray[j][1]<<"   ###     ";
       ...
       //         cout<<endl;
                ...
                if(a>1){
						PPamp[g1][s1][i]=parray[a-2][1];
                        PPmean[g1][s1][i]=parray[a-2][0];
				}
        ...
        //        cout<<"Fitting parameters: "<< g1<<"\t"<<s1<<"\t"<<i<<"\t"<<"  #########   "<<fG->GetParameter(0)<<"\t"<<fG->GetParameter(1)<<"\t"<<fG->GetParameter(2)<<endl;
          ...
          if((g1==0)||(g1==1)){
			...
			///////////////####################################### second peak
			if(a>1){
				PPleft = float(PPmean[g1][s1][i])*(1-sigP/8);
                PPright = float(PPmean[g1][s1][i])*(1+sigP/8);
                fG = new TF1("fG","[0]*exp(-0.5*((x-[1])/[2])^2)",PPleft,PPright);
				gsigma=PPmean[g1][s1][i]*0.12/2.355;
				fG->SetParameters(10.0,PPmean[g1][s1][i],gsigma);
				Sig[g1][s1][i]->Fit(fG,"RQ+");
                
                R1=fG->GetParameter(1)-abs(3*fG->GetParameter(2));
				R2=fG->GetParameter(1)+abs(3*fG->GetParameter(2));
				st=abs(6*fG->GetParameter(2))/1000;
				rb1=R1/4;
				rb2=R2/4;
                ...
                if(((a>0)&&(a<=7))&&((abs(fG->GetParameter(2))<sigB*2))&&((peak>=peakLT)&&(peak<=peakHT))){                                                                                    //// condition for good spectrum and good fit
                out2<<g1<<"\t"<<s1<<"\t"<<i<<"\t"<<peak<<"\t"<<fG->GetParameter(1)<<"\t"<<abs(fG->GetParameter(2))<<"\t"<<realE2<<endl;
				if((g1==0)||(g1==1)){
						h2PeakC0->SetBinContent(px1+1,py1+1,peak);
				}else{
						h2PeakC1->SetBinContent(px1+1,py1+1,peak);	
				}
				}else{
				out2<<g1<<"\t"<<s1<<"\t"<<i<<"\t"<<0.0<<"\t"<<0.0<<"\t"<<0.0<<"\t"<<0.0<<endl;
					if((g1==0)||(g1==1)){
						h2PeakC0->SetBinContent(px1+1,py1+1,0);
					}else{
						h2PeakC1->SetBinContent(px1+1,py1+1,0);	
					}
				}
    ...
    TCanvas *cpeak2C= new TCanvas("cpeak2C","cpeak2C",1000,1000);
    gStyle->SetPalette(55);
    cpeak2C->Divide(1,2);
    cpeak2C->cd(1);
    h2PeakC0->Draw("colz");
    h2PeakC0->GetXaxis()->SetTitle("X index");
    h2PeakC0->GetYaxis()->SetTitle("Y index");
     cpeak2C->cd(2);
    h2PeakC1->Draw("colz");
    h2PeakC1->GetXaxis()->SetTitle("X index");
    h2PeakC1->GetYaxis()->SetTitle("Y index");
    cpeak2C->SaveAs("EBOX/Peak2nd_Selected__2D_v1.pdf");
    cpeak2C->SaveAs("EBOX/Peak2nd_Selected_2D_v1.png");
    cpeak2C->Write();
